...
메인 로직인 pms_main.R에 보면 사용자가 설정하는 란이 있다. 아래에서 wd(작업디렉토리)는 그대로 두는 것이 좋고 fund_name은 사용자가 펀드 이름을 지어서 넣으면 된다. 나중에 1페이지 보고서의 타이틀에 펀드명이 나온다. weights는 만약 아래의 ETF를 그대로 사용한다고 가정했을때 비중으로 합해서 1이 된다면 사용자의 입맛에 맞게 수정하면 된다. REPEAT_FLAG는 주기적으로 반복하면서 루프를 돌게 할 때 TRUE로 바꾸면 되지만 특별한 경우가 아니면 FALSE로 두면 된다.
...
그리고 안정적인 quantmod 패키지도 가끔 yahoo의 오류로 에러를 낼 때가 있다. 예를 들어 "Unable to import “O" 커넥션을 열 수 없습니다"라는 에러메시지는 아주 간혹 나올 때가 있는데 약간 기다리면 자동으로 해결되는 경우가 많다. 아무래도 무료로 제공하다보니 많은 사람들이 조회를 하거나 시스템의 불안정으로 인해 그렇게 나오는 것 같다. 이런 경우 너무 놀라지 말고 잠시 기다렸다가 조회하면 된다.
...
여기서 빈 문자열이 나오면, 등록이 안 된 것이다. 그리고 한 가지 더 중요한 함정이 있다. 환경 변수를 등록했는데도 RStudio에서 빈 값이 나오는 경우가 있는데, 그건 RStudio가 이미 켜져 있었기 때문이다. 환경 변수는 프로그램을 다시 실행해야 반영된다. 그러니 환경 변수를 등록한 뒤에는 RStudio를 완전히 종료했다가 다시 켜야 한다. 이 단순한 사실 하나 때문에 많은 초보자가 하루를 날린다. 참고로 윈도우 환경 변수 등록을 한 뒤에는 Rstudio를 종료한 후 다시 기동해야 인식이 잘 된다.
...
마지막으로 프롬프트 엔지니어링에 대해 살펴보자. pms_main.R의 내용에 보면 make_gemini_prompt_pms() 이름의 함수가 정의 되어 있다. 아래에 보면 프롬프트를 볼 수 있는데 여기에 보면 제미나이 생성형 AI에 게 질의할 자연어 프롬프트가 나오는데 paste0()함수에 나와 있는 프롬프트(AI 질의어)를 적절히 잘 조정하면 원하는대로 메일의 본문 내용을 써준다. 만약 아래 내용이 마음에 들지 않으면 독자 여러분이 마음대로 '자연어'로 바꿔쓰면 된다. 그렇지만 너무 길거나 복잡하게 쓰면 제미나이가 잘 대답할 수 없으니 가급적 간단하고 명료하게 프롬프트를 작성하면 된다.
...
여기까지 잘 따라왔다면 실행하는 자체는 큰 무리없을 것으로 믿는다. 

그리고 gemini_ai_email.R에 보면 아래와 같은 수신인 이메일 적는 부분이 나온다.
아래의 to 변수는 수신인 이메일을 적는 부분인데 필자 이메일인 seminago@naver.com으로 적혀 있으니 독자여러분이 수신하는 이메일로 바꾸어야 한다.
```
# ------------------------------------------------------------
...
이 챕터까지 따라하시느라 독자 여러분 수고 많으셨다. 성공적 투자를 기원한다.
