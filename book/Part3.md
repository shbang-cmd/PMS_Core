...
아래는 필자가 추천하는 PMS 표준 폴더 구조다. 당신이 윈도우를 쓰든, 맥을 쓰든, 이 구조는 그대로 유지하는 것이 좋다. 각 트리 구조 오른쪽에 어떤 파일인지 설명을 자세히 달아 놓았다.
...
위에 나오지 않은 나머지 파일들은 임시파일이거나 각종 시뮬레이션을 위한 R파일, 로그폴더의 로그파일 등이다.
