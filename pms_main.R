  ...
  tryCatch({
    ...
    if (!goto_sleep) {
      ...
      if (!risk_ready) {
        ...
        # =========================================================
        # VaR과 CVaR 설명
        # - VaR (Value at Risk) : "최악의 상황에서 이 정도는 잃을 수 있어"
        #   예시) VaR : 약 6.78%(83,785,569원) CVaR : 약 8.91%(110,035,957원)으로 나왔다면
        # 먼저 VaR(95%)가 약 6.78%(83,785,569원)라는 말의 의미
        #        여기서 '95% 신뢰수준'이라는 말은, 우리가 100개월 동안 투자를 한다면 그중 아주 평범하거나 적당히 나쁜 95개월 동안은 손실이 이 금액을 넘지 않을 것이라는 뜻. 뒤집어서 말하면, "정말 운이 없는 하위 5%의 상황(한 달에 1번 꼴)"이 닥쳤을 때, 최소한 8,378만 원(전체 자산의 6.78%) 정도의 손실이 시작되는 지점이라는 의미.
        #        비유: 학교 시험 성적으로 비유하자면, 전교생 100명 중 뒤에서 5등을 한 학생의 점수라고 생각하면 됩니다. "내 성적이 아무리 안 나와도 뒤에서 5등인 이 점수보다는 잘 나올 확률이 95%야"라고 말할 때의 그 기준점.
        #  - CVaR (Conditional Value at Risk) : "하필 그 5% 안에 들었을 때, 진짜 평균적으로 얼마나 깨질까?"
        #        다음은 CVaR(95%)가 약 8.91%(110,035,957원)라는 지표로 펀드매니저들은 VaR보다 이 CVaR를 더 무섭게 여김.
        #                 VaR가 단순히 '하위 5%가 시작되는 입구'의 값이라면, CVaR는 "그 입구를 지나쳐버린 아주 끔찍한 5%의 상황들만 모아서 평균을 낸 값". 즉, 운이 너무 없어서 하위 5% 구간에 진입했을 때, 실제로 우리가 겪게 될 손실의 평균값이 약 1억 1,003만 원(전체 자산의 8.91%)이라는 뜻.
        #        비유: 아까 말한 전교 꼴찌 그룹(하위 5명)의 상황으로 다시 가보면 뒤에서 5등인 학생의 점수가 VaR라면, CVaR는 뒤에서 1등, 2등, 3등, 4등, 5등인 학생들의 점수를 모두 합해 평균을 낸 값. 당연히 입구(5등) 점수보다는 평균(1~5등 평균) 점수가 더 낮고 처참함. 그래서 CVaR는 항상 VaR보다 큰 손실액을 나타냄.
        
        #  비중의 개수가 맞다면 강제로 이름을 부여하여 분석의 연속성을 확보
